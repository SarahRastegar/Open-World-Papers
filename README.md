...
### Unknown Categories
Telsa scientists are relieved that their autopilot model is ready to be employed in real world. Their model can handle unseen environments and unseen actions and categories. However, there is still a minor problem, there is one extremely likely prediction that one day their autopilot will encounter objects or actions that even they are not aware of at training time. So they decide that they should train their model to make them aware anytime it sees something new or simply reject it. However they are not how to regulate this uncertainty. Some scientists suggest that anything which was from the training time distribution should be rejected. In computer vision this problem is known as [Out-of-Distribution Detection](#Out-of-Distribution-Detection) or its video version as [Out-of-Distribution Detection Videos](#Out-of-Distribution-Detection-Videos). Another highly overlapped scenario is 
[Anomaly Detection](#Anomaly-Detection) or its video version [Anomaly Detection Videos](#Anomaly-Detection-Videos). In Anomaly detection the anomal object or event usually is rare and we are interested to extract them from data, for OOD though the rejected samples are usually abundant and the rejection can be done automatically. 

Sometimes it is important to reject data that we do not know their category while they could be in-distribution. This problem which is called [Open-Set Recognition](#Open-Set-Recognition) and its video [Open-Set Recognition Videos](#Open-Set-Recognition-Videos). 

The problem with OOD or OSR is that the model rejects the unknowns, so though it could update itself to become better but it does not learn the new categories. This is in direct contrast with humans which actually start which categories new objects or actions. So if an agent is able to detect these novel categories and cluster them, it could use this novel knowledge to its benefit and use from the vast number of novel but unlabelled samples. This problem is called [Novel Class Discovery](#Novel-Class-Discovery) or its video version [Novel Class Discovery Videos](#Novel-Class-Discovery-Videos). 
